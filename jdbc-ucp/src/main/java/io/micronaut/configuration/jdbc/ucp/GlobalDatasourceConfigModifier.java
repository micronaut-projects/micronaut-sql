...
public class GlobalDatasourceConfigModifier implements BeanCreatedEventListener<DatasourceConfiguration> {
    ...
    public DatasourceConfiguration onCreated(BeanCreatedEvent<DatasourceConfiguration> event) {
        ...
        Properties existingProperties = configuration.delegate.getConnectionProperties();
        if (existingProperties == null) {
            existingProperties = new Properties();
        }

        final Properties finalExistingProperties = existingProperties;
        globalDatasourceProperties.getDataSourceProperties()
            .forEach((key, value) -> {
                    if (value != null && !finalExistingProperties.containsKey(key)) {
                        finalExistingProperties.setProperty(key, value.toString());
                    }
                }
            );

        try {
            configuration.delegate.setConnectionProperties(finalExistingProperties);
        } catch (SQLException e) {
            throw new ConfigurationException("Unable to set global datasource properties: " + e.getMessage(), e);
        }
