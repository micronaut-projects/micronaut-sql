Java data sources can be configured for one of four currently provided pooled implementations. Apache DBCP2, Hikari, Tomcat, and Oracle Universal Connection Pool are supported by default. Additionally, unpooled datasources can be enabled via configuration.
...
----
$ mn create-app my-app --features jdbc-hikari
----
====
...
NOTE: Unpooled datasources are also available by setting `datasources.allow-unpooled=true` in your configuration when using the `micronaut-jdbc` module. The unpooled datasource creates a new connection for each request and should only be used for testing, serverless, or very low-volume applications or when using a database proxy like CNAM, pgpool or Open J Proxy. See the link:jdbc-unpooled.html[Unpooled JDBC DataSources] documentation for details.
