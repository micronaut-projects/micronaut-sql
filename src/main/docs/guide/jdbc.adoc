Java data sources can be configured for one of five currently provided implementations. Apache DBCP2, Hikari, Tomcat, Oracle Universal Connection Pool, and Unpooled are supported by default.
...
WARNING: The unpooled datasource creates a new connection for each request and should only be used for testing, serverless, or very low-volume applications or when using a database proxy like CNAM, pgpool or Open J Proxy. See the link:jdbc-unpooled.html[Unpooled JDBC DataSources] documentation for details.
