Micronaut supports reactive and non-blocking client to connect to Postgres using https://github.com/eclipse-vertx/vertx-sql-client/blob/master/vertx-pg-client[vertx-pg-client], allowing to handle many database connections with a single thread.
