...
- Default value is taken from `micronaut.application.name`. If not present, it falls back to `"Micronaut"`.
...
datasources:
  default:
    ...
    username: user
    password: pass
    dialect: ORACLE   # Auto-sets v$session.program = "my-app"
  ...
  disabled:
    ...
    oracle:
      session:
        enabled: false   # v$session.program not auto-set
----
